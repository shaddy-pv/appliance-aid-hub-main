...
const CheckoutPage = () => {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    if (values.paymentMethod === "cod") {
      ...
      toast({ title: "Order placed", description: `Order ${order.id} placed successfully. Payment on delivery.` });
      reset();
      clear();
      navigate("/");
    } else if (values.paymentMethod === "card" || values.paymentMethod === "upi") {
        ...
        await PaymentService.initializePayment({
          ...
          onSuccess: (paymentId) => {
            toast({ 
              ...
              description: `Order ${order.id} placed and payment completed successfully!` 
            });
            reset();
            clear();
            navigate("/");
          ...
          <CardContent className="p-6">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div>
                ...
                {watch("paymentMethod") !== "cod" && !isRazorpayLoaded && (
                  <p className="text-sm text-muted-foreground mt-1">Loading payment system...</p>
                )}
              </div>
