...
function setAuthCookies(res: any, accessToken: string, refreshToken: string) {
  ...
  res.cookie('aah_access', accessToken, { 
    httpOnly: true, 
    sameSite: 'lax', 
    secure: isProd, 
  ...
  res.cookie('aah_refresh', refreshToken, { 
    httpOnly: true, 
    sameSite: 'lax', 
    secure: isProd, 
    maxAge: 1000 * 60 * 60 * 24 * Number(process.env.REFRESH_TOKEN_TTL_DAYS || '14'), 
    domain 
  });
}
...
authRouter.post('/refresh', async (req, res) => {
  ...
  for (const t of tokens) {
    ...
    if (match && !t.revokedAt && t.expiresAt > new Date()) { 
      owner = t.userId.toString(); 
      break; 
    }
  }
